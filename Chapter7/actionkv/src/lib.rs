...
impl ActionKV {
    ...
    pub fn process_record<R: Read>(f: &mut R) -> io::Result<KeyValuePair> {
        ...
        // in block to avoid scoping issues
        {
            let _ = f.by_ref().take(data_len as u64).read_to_end(&mut data)?;
        }
        debug_assert_eq!(data.len(), data_len as usize);
        // should use crypto checksum for production code
        let checksum = crc32::checksum_ieee(&data);
        if checksum != saved_checksum {
            panic!(
                "Data corrupted (checksum): ({:08x} != {:08x})",
