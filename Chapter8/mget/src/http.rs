...
pub fn get(
    ...
    let domain_name = url.host().ok_or(UpstreamError::InvalidUrl)?;
    let neighbour_cache = NeighborCache::new(BTreeMap::new());
    ...
    let mut iface = EthernetInterfaceBuilder::new(tap)
        .ethernet_addr(mac)
        .neighbor_cache(neighbour_cache)
        .ip_addrs(ip_addrs)
        .routes(routes)
        .finalize();
...

        {
            ...
            state = match state {
                ...
                HttpState::Request if socket.may_send() => {
                    ...
                    socket.send_slice(http_handler.as_ref())?;
                    HttpState::Response
                }
